...
Seis de las diez abren un navegador real (para probar el modo sin señal, el
...
Sin eso, esas seis se saltean y las otras cuatro corren igual.
...
| --- | --- |
| `probar.js` | El recorrido completo: la web sigue funcionando, y en la app el ingreso, el nombre del día, los tres pasos con todas sus validaciones, la numeración, el ticket, el PDF, los pedidos de anulación, los permisos y **de quién queda cada ticket** (el campo no le cambia el dueño). |
...
| `probar-sin-senal-real.js` | Sin señal **con el service worker de verdad**, que es lo que corre en el teléfono: abrir la app desde el ícono, seguir un ticket que ya está en el servidor, ver el ticket, cargar de cero, el aviso de pesadas sin subir en todas las pantallas y con cualquier código, y que al cambiar de código no queden pantallas de la sesión anterior. |
| `probar-reporte-email.js` | El reporte de las 19 hs: abre el Excel generado y lee las celdas de la hoja **Acumulado campaña** (que sume por lote y por grano, que no cuente anulados ni camiones sin regular, y el corte del 1 de septiembre). Y lo más importante: rompe el acumulado a propósito y comprueba que **el reporte de todos los días se mande igual**. Nunca manda un mail de verdad. |
